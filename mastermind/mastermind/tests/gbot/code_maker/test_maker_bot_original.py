...
class TestMakerBotOriginal:
    ...
    def test_create_validation_correct_guess(self, maker):
        maker.code = [Colors.RED, Colors.BLUE, Colors.YELLOW, Colors.GREEN]
        guess = [Colors.RED, Colors.BLUE, Colors.YELLOW, Colors.GREEN]
        expected_rating = [Colors.BLACK, Colors.BLACK, Colors.BLACK, Colors.BLACK]
        assert maker.create_validation(guess) == expected_rating

    def test_create_validation_correct_colors_wrong_positions(self, maker):
        maker.code = [Colors.RED, Colors.BLUE, Colors.YELLOW, Colors.GREEN]
        guess = [Colors.GREEN, Colors.YELLOW, Colors.BLUE, Colors.RED]
        expected_rating = [Colors.WHITE, Colors.WHITE, Colors.WHITE, Colors.WHITE]
        assert maker.create_validation(guess) == expected_rating

    def test_create_validation_incorrect_colors(self, maker):
        maker.code = [Colors.RED, Colors.BLUE, Colors.YELLOW, Colors.GREEN]
        guess = [Colors.WHITE, Colors.WHITE, Colors.WHITE, Colors.WHITE]
        expected_rating = []
        assert maker.create_validation(guess) == expected_rating

    def test_create_validation_mixed_rating(self, maker):
        maker.code = [Colors.RED, Colors.BLUE, Colors.YELLOW, Colors.GREEN]
        guess = [Colors.RED, Colors.YELLOW, Colors.GREEN, Colors.BLUE]
        expected_rating = [Colors.BLACK, Colors.WHITE, Colors.WHITE, Colors.WHITE]
        assert maker.create_validation(guess) == expected_rating
    ...
    def test_create_validation_event(self, maker):
        ...
        maker.code = [Colors.WHITE, Colors.RED, Colors.BLUE, Colors.GREEN]
        guess = [Colors.RED, Colors.RED, Colors.GREEN, Colors.BLUE]
        rating = maker.create_validation(guess)

        assert rating == [Colors.BLACK, Colors.WHITE, Colors.WHITE]
        mock_pub.sendMessage.assert_called_once_with("add_rating", rating=[Colors.BLACK, Colors.WHITE, Colors.WHITE])
